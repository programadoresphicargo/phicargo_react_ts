...
const SolicitudForm = ({ id_solicitud, open, handleClose, onSaveSuccess }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            setLoading(true);
            const response = await odooApi.get(`/tms_travel/asignaciones_equipo/id_asignacion/${id_solicitud}`);
            setData(response.data);
            setEPP(response.data.equipos);
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (id_solicitud === null) {
                ...
                const response = await odooApi.patch(`/tms_travel/asignaciones_equipo/${id_solicitud}`, payload);
                if (response.data.status == 'success') {
                    toast.success(response.data.message);
                } else {
                    toast.error(response.data.message);
    ...
    const confirmar = async () => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                setLoading(true);
                const response = await odooApi.get('/tms_travel/asignaciones_equipo/confirmar/' + id_solicitud);
                if (response.data.status == 'success') {
                    toast.success(response.data.message);
                    fetchData();
                    handleClose();
            ...
            {isLoading ? (
                ...
                <DialogContent>
                    ...
                    <Stack spacing={1} direction="row" className="mb-5">
                        ...
                        {data?.estado === "borrador" && !modoEdicion && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="md:col-span-2 grid grid-cols-1 gap-4">
                            ...
                            <Card>
                                ...
                                <Divider></Divider>
                                <CardBody>
                                    <h2><strong>Creado por:</strong> {data?.nombre}</h2>
                                    <h2><strong>Fecha de solicitud:</strong> {data?.fecha_creacion}</h2>
                                    <SelectOperador
                                        label={'Operador'}
                                        id={'id_operador'}
                                        name={'id_operador'}
                                        onChange={handleSelectChange}
                                        value={data?.id_operador}
