...
const KMRecorridosOperadores = () => {
  ...
  const fetchData = async () => {
    try {
      setisLoading(true);
      const response = await odooApi.get(`/drivers/stats/?start_date=${range[0].toISOString().slice(0, 10)}&end_date=${range[1].toISOString().slice(0, 10)}`);
