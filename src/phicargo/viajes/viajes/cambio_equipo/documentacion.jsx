...
const IndexCambioEquipo = ({ }) => {
  ...
  const registrar_cambio = async () => {
    ...
    try {
      const response = await fetch('/phicargo/viajes/cambios/guardar.php', {
        method: 'POST',
        body: Data,
      });
  ...
  useEffect(() => {
    if (id_viaje) {
      axios.get(`/phicargo/viajes/cambios/getEquipo.php?id_viaje=${id_viaje}`)
        .then((response) => {
      ...
      axios.get(`/phicargo/viajes/cambios/getCartas.php?id_viaje=${id_viaje}`)
        .then((response) => {
