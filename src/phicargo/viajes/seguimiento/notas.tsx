...
export default function NotasViaje({
 open,
 onClose,
 id_viaje,
}: Props) {
 const initial: ViajeForm = {
  travel_id: id_viaje,
  note: "",
 };
 ...
 const [data, setData] = useState<ViajeForm[]>([]);

 /**
  * Formatea la fecha recibida desde el backend.
  *
  * Ejemplo:
  * 2026-08-13T16:08:28.920678
  *
  * Resultado:
  * 13 ago. 2026 · 10:08 a. m.
  */
 ...
 const fetchData = async () => {
  try {
   ...
   const res = await odooApi.get(
    `/tms_travel/notes/travel_id/${id_viaje}`
   );
  ...
  */
 const SaveForm = async (formData: ViajeForm) => {
  try {
   setLoadingCreate(true);

   const url = "/tms_travel/notes/";
     ...
     <div className="mt-5">
      ...
      {!isLoading && data.length === 0 && (
       <div className="flex flex-col items-center justify-center py-10 text-center">
        ...
        <p className="text-xs text-gray-400 mt-1">
         La primera nota de este viaje aparecerá aquí.
        </p>
       </div>
      )}
      ...
      {!isLoading && data.length > 0 && (
       <div className="space-y-3">
        {data.map((item, index) => (
         <Card
          key={`${item.travel_id}-${item.created_date}-${index}`}
