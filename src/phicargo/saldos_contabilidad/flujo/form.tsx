...
const FlujoForm = ({ open, handleClose, Cuenta, paymentId }: Props) => {
  ...
  useEffect(() => {
    const costoIndex = details.findIndex(
      d => d.category_id === 2
    );

    if (costoIndex === -1) return;

    const iva = Number((details[costoIndex].amount * 0.16).toFixed(2));

    const impuestoIndex = details.findIndex(
      d => d.category_id === 5
    );
    ...
    if (details[impuestoIndex].autoGenerated) {
      update(impuestoIndex, {
        ...details[impuestoIndex],
        amount: iva,
      });
    }
  }, [details]);
